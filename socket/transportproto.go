...
type TransportSocket interface {
	...
	// Writes the complete block from blockContext.Data in the way this interface likes to
	...
	// Read the complete block into blockContext.Data
	...
	// Connects to the addr, so all WriteBlock and Write calls send to this destination
	Dial(addr string) error
}

type TransportPacketPacker interface {
	// Blocks needs to know how large the header is
